...
## 2024-05-29 - Enhance Keyboard Shortcut Discoverability and Screen Reader Context
**Learning:** Decorative icons placed adjacent to descriptive text in buttons can cause screen readers to redundantly announce the icon's semantic name, creating an awkward auditory experience. Furthermore, while global keyboard shortcuts (like `Escape` to close modals) exist in the code, users often remain unaware of them without explicit visual hints.
**Action:** Always apply `aria-hidden="true"` to purely decorative icons that accompany descriptive text labels within interactive elements. Additionally, expose supported keyboard shortcuts natively via `title` and `aria-label` attributes on relevant action buttons (e.g., appending `(Esc)` to a close button's label) to intuitively teach users power-user workflows without requiring separate documentation.
