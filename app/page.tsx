  ...
  useEffect(() => {
    const processInput = () => {
      ...
      try {
        ...
        const supportedDomains = ['youtube.com', 'youtu.be', 'twitter.com', 'x.com', 'linkedin.com', 'instagram.com', 'facebook.com', 'fb.com', 'tiktok.com', 'spotify.com'];
        
        const isSupportedDomain = supportedDomains.some(domain => urlObj.hostname.toLowerCase().includes(domain));
        
        if (!isSupportedDomain) {
          setParsed(null);
          setError("Platform not supported. We currently support YouTube, X, LinkedIn, Instagram, Facebook, TikTok, and Spotify.");
          return;
        }
              ...
              <div className="absolute right-3 md:right-4 top-1/2 -translate-y-1/2 flex items-center p-1.5 rounded-2xl bg-[#050505]/60 backdrop-blur-xl border border-white/10 shadow-2xl">
                ...
                <div 
                  ...
                  style={{ 
                    ...
                    backgroundColor: parsed ? `${parsed.color}20` : error ? 'rgba(239,68,68,0.15)' : 'rgba(255,255,255,0.05)' 
                  }}
                >
                   {error ? <AlertCircle className="w-6 h-6 md:w-7 md:h-7" /> : <PlatformIcon platform={parsed ? parsed.platform : 'unknown'} className="w-6 h-6 md:w-7 md:h-7" />}
                </div>
              </div>
            </motion.div>
            ...
            <AnimatePresence>
              {error && (
                ...
                >
                  ...
                  <span className="text-left md:text-center text-sm md:text-base leading-snug">{error}</span>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {parsed && (
              ...
              >
                <div className="bg-white/5 border border-white/10 rounded-3xl p-8 md:p-10 flex flex-col lg:flex-row items-center justify-between gap-10 relative group shadow-2xl">
                  ...
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-3xl" style={{ backgroundImage: `radial-gradient(circle at center, ${parsed.color}25 0%, transparent 70%)` }} />
                  
                  <div className="flex items-center gap-6 z-10 w-full lg:w-auto flex-1 min-w-0">
                    <div className="relative shrink-0">
                      <div className="w-24 h-24 rounded-3xl bg-white/10 flex items-center justify-center border border-white/20 shadow-2xl backdrop-blur-md relative overflow-hidden" style={{ color: parsed.color }}>
                        {previewData?.image ? (
                          <img src={previewData.image} alt="Preview" className="absolute inset-0 w-full h-full object-contain opacity-90" />
                        ) : (
                          <>
                            <div className="absolute inset-0 opacity-25" style={{ backgroundColor: parsed.color }} />
                            <PlatformIcon platform={parsed.platform} className="w-12 h-12 relative z-10 drop-shadow-xl" />
                          </>
                        )}
                      </div>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-4 mb-2">
                        <h3 className="font-bold text-3xl capitalize tracking-tight text-white/95 truncate">{parsed.platform}</h3>
                        <span className="shrink-0 whitespace-nowrap px-3 py-1 rounded-full bg-green-500/15 text-green-400 text-[11px] uppercase tracking-widest font-bold border border-green-500/30 shadow-lg shadow-green-500/10">Certified App</span>
                      </div>
                      {isLoadingPreview ? (
                        <div className="flex items-center gap-3 mt-3">
                          <div className="h-5 w-5 rounded-full border-2 border-white/20 border-t-white/90 animate-spin" />
                          <p className="text-white/70 text-base font-medium">{loadingText}</p>
                        </div>
                      ) : previewData?.title ? (
                        <p className="text-white/90 text-base font-medium leading-relaxed">{previewData.title}</p>
                      ) : (
                        <p className="text-white/50 text-sm break-all font-mono opacity-80">{parsed.originalUrl}</p>
                      )}
                    </div>
                  </div>
